  ...
  "challenges": [
    ...
    },
    {
      ...
      "variants": [
        ...
        { "id": "v3", "title": "Count Passing Scores", "description": "Write <code>func countPassing(scores []int, threshold int) int</code> that counts scores >= threshold.", "functionSignature": "func countPassing(scores []int, threshold int) int", "testCases": [{ "input": "[]int{55, 72, 68, 90, 45}, 60", "output": "3" }], "hints": [{ "title": "🤔 Think about it", "content": "You need to count things that meet a condition. What two pieces do you need: one to track the count, and one to check each element?" }, { "title": "💡 Hint", "content": "Start a counter at 0. For each element, check your condition - if true, increment the counter." }, { "title": "🔧 Pattern", "content": "<pre>1. Start counter at 0\n2. For each element:\n   - Meets condition? → increment counter\n3. Return counter</pre>" }], "solution": "func countPassing(scores []int, threshold int) int {\n    count := 0\n    for _, score := range scores {\n        if score >= threshold {\n            count++\n        }\n    }\n    return count\n}" }
      ]
    },
    {
    ...
    },
    {
      ...
      "variants": [
        ...
        { "id": "v4", "title": "Count Zeros", "description": "Write <code>func countZeros(nums []int) int</code> that returns how many zeros are in the slice.", "functionSignature": "func countZeros(nums []int) int", "testCases": [{ "input": "[]int{0, 1, 0, 2, 0}", "output": "3" }, { "input": "[]int{1, 2, 3}", "output": "0" }], "hints": [{ "title": "🤔 Think about it", "content": "You don't need a map here - just a simple counter variable. When should you increment it?" }, { "title": "💡 Hint", "content": "Start with count := 0. Loop through the slice. If the element equals 0, increment count." }, { "title": "🔧 Pattern", "content": "<pre>1. Initialize count := 0\n2. For each element:\n   - Is it 0? → count++\n3. Return count</pre>" }], "solution": "func countZeros(nums []int) int {\n    count := 0\n    for _, n := range nums {\n        if n == 0 { count++ }\n    }\n    return count\n}" },
        { "id": "v5", "title": "Count Positives and Negatives", "description": "Write <code>func countPosNeg(nums []int) (int, int)</code> returning two values: count of positive numbers and count of negative numbers. Zero is neither.", "functionSignature": "func countPosNeg(nums []int) (int, int)", "testCases": [{ "input": "[]int{-2, -1, 0, 1, 2, 3}", "output": "3, 2", "note": "3 positive (1,2,3), 2 negative (-2,-1)" }], "hints": [{ "title": "🤔 Think about it", "content": "You need two counters. When do you increment each one?" }, { "title": "💡 Hint", "content": "Use pos, neg := 0, 0. Check if n > 0 (positive) or n < 0 (negative). Return both at the end." }, { "title": "🔧 Pattern", "content": "<pre>1. Initialize pos, neg := 0, 0\n2. For each element:\n   - n > 0? → pos++\n   - n < 0? → neg++\n3. Return pos, neg</pre>" }], "solution": "func countPosNeg(nums []int) (int, int) {\n    pos, neg := 0, 0\n    for _, n := range nums {\n        if n > 0 { pos++ } else if n < 0 { neg++ }\n    }\n    return pos, neg\n}" },
        { "id": "v6", "title": "Count Even and Odd", "description": "Write <code>func countEvenOdd(nums []int) map[string]int</code> returning a map with keys \"even\" and \"odd\" showing how many of each.", "functionSignature": "func countEvenOdd(nums []int) map[string]int", "testCases": [{ "input": "[]int{1, 2, 3, 4, 5}", "output": "map[even:2 odd:3]" }], "hints": [{ "title": "🤔 Think about it", "content": "Similar to counting positives/negatives, but now store the counts in a map instead of separate variables." }, { "title": "💡 Hint", "content": "Use num % 2 to check: if it equals 0, the number is even. Increment the appropriate key in your map." }, { "title": "🔧 Pattern", "content": "<pre>1. Create empty count map\n2. For each element:\n   - Is it even (num % 2 == 0)? → increment \"even\"\n   - Otherwise → increment \"odd\"\n3. Return map</pre>" }], "solution": "func countEvenOdd(nums []int) map[string]int {\n    c := make(map[string]int)\n    for _, n := range nums {\n        if n % 2 == 0 { c[\"even\"]++ } else { c[\"odd\"]++ }\n    }\n    return c\n}" },
        { "id": "v7", "title": "Most Frequent", "description": "Write <code>func mostFrequent(nums []int) int</code> returning the number that appears most often. Assume non-empty input with a unique most frequent.", "functionSignature": "func mostFrequent(nums []int) int", "testCases": [{ "input": "[]int{1, 2, 2, 3, 3, 3}", "output": "3" }, { "input": "[]int{5, 5, 1, 2}", "output": "5" }], "hints": [{ "title": "🤔 Think about it", "content": "This is a two-step problem: first count occurrences (you know how!), then find which key has the highest value." }, { "title": "💡 Hint", "content": "First build a count map like before. Then loop through the map to find which key has the maximum value." }, { "title": "🔧 Pattern", "content": "<pre>1. Build count map (same pattern as before)\n2. Track maxCount and maxNum\n3. For each key, value in map:\n   - If value > maxCount → update maxCount and maxNum\n4. Return maxNum</pre>" }], "solution": "func mostFrequent(nums []int) int {\n    c := make(map[int]int)\n    for _, n := range nums { c[n]++ }\n    maxNum, maxCount := 0, 0\n    for num, count := range c {\n        if count > maxCount { maxCount, maxNum = count, num }\n    }\n    return maxNum\n}" },
        { "id": "v8", "title": "Count by Length", "description": "Write <code>func countByLength(words []string) map[int]int</code> that groups words by their length and counts how many words are in each group. Keys are lengths, values are counts.", "functionSignature": "func countByLength(words []string) map[int]int", "testCases": [{ "input": "[]string{\"go\", \"rust\", \"c\", \"js\"}", "output": "map[1:1 2:2 4:1]", "note": "1 word of length 1 (c), 2 words of length 2 (go, js), 1 word of length 4 (rust)" }], "hints": [{ "title": "🤔 Think about it", "content": "Instead of counting the words themselves, you're counting a property of each word. What property? Its length." }, { "title": "💡 Hint", "content": "Use len(word) as the key. Same counting pattern, just using the length instead of the word itself." }, { "title": "🔧 Pattern", "content": "<pre>1. Create empty count map\n2. For each word:\n   - Get its length with len(word)\n   - Increment count for that length\n3. Return map</pre>" }], "solution": "func countByLength(words []string) map[int]int {\n    c := make(map[int]int)\n    for _, w := range words { c[len(w)]++ }\n    return c\n}" }
      ]
    },
    {
      ...
      "variants": [
        ...
        { "id": "v4", "title": "Swap First Two", "description": "Write <code>func swapFirstTwo(nums []int)</code> that swaps the elements at index 0 and index 1. Assume slice has at least 2 elements.", "functionSignature": "func swapFirstTwo(nums []int)", "testCases": [{ "input": "[]int{1,2,3,4}", "output": "[2,1,3,4]" }, { "input": "[]int{5,10}", "output": "[10,5]" }], "hints": [{ "title": "🤔 Think about it", "content": "This is just one swap - no loop needed. What indices do you need?" }, { "title": "💡 Hint", "content": "Swap nums[0] and nums[1] using simultaneous assignment." }, { "title": "🔧 Pattern", "content": "<pre>nums[0], nums[1] = nums[1], nums[0]</pre>" }], "solution": "func swapFirstTwo(nums []int) {\n    nums[0], nums[1] = nums[1], nums[0]\n}" },
        { "id": "v5", "title": "Swap Middle Two", "description": "Write <code>func swapMiddle(nums []int)</code> that swaps the two middle elements. Assume even length.", "functionSignature": "func swapMiddle(nums []int)", "testCases": [{ "input": "[]int{1,2,3,4}", "output": "[1,3,2,4]" }, { "input": "[]int{10,20,30,40,50,60}", "output": "[10,20,40,30,50,60]" }], "hints": [{ "title": "🤔 Think about it", "content": "For a slice of length n, the two middle elements are at indices n/2-1 and n/2." }, { "title": "💡 Hint", "content": "Calculate mid := len(nums)/2. Then swap nums[mid-1] and nums[mid]." }, { "title": "🔧 Pattern", "content": "<pre>mid := len(nums) / 2\nnums[mid-1], nums[mid] = nums[mid], nums[mid-1]</pre>" }], "solution": "func swapMiddle(nums []int) {\n    mid := len(nums) / 2\n    nums[mid-1], nums[mid] = nums[mid], nums[mid-1]\n}" },
        { "id": "v6", "title": "Swap at Offset", "description": "Write <code>func swapAtOffset(nums []int, k int)</code> that swaps element at index k with element at index len-1-k (mirror positions from each end).", "functionSignature": "func swapAtOffset(nums []int, k int)", "testCases": [{ "input": "[]int{1,2,3,4,5}, 1", "output": "[1,4,3,2,5]", "note": "swaps index 1 with index 3" }, { "input": "[]int{1,2,3,4,5}, 0", "output": "[5,2,3,4,1]", "note": "swaps index 0 with index 4 (same as swapEnds)" }], "hints": [{ "title": "🤔 Think about it", "content": "The mirror position of index k from the start is len-1-k from the end." }, { "title": "💡 Hint", "content": "Swap nums[k] with nums[len(nums)-1-k]." }, { "title": "🔧 Pattern", "content": "<pre>nums[k], nums[len(nums)-1-k] = nums[len(nums)-1-k], nums[k]</pre>" }], "solution": "func swapAtOffset(nums []int, k int) {\n    nums[k], nums[len(nums)-1-k] = nums[len(nums)-1-k], nums[k]\n}" },
        { "id": "v7", "title": "Swap Min and Max", "description": "Write <code>func swapMinMax(nums []int)</code> that finds the minimum and maximum elements and swaps their positions.", "functionSignature": "func swapMinMax(nums []int)", "testCases": [{ "input": "[]int{3,1,4,1,5}", "output": "[3,5,4,1,1]" }, { "input": "[]int{5,2,8,1}", "output": "[5,2,1,8]" }], "hints": [{ "title": "🤔 Think about it", "content": "First you need to find WHERE the min and max are (their indices), then swap the elements at those positions." }, { "title": "💡 Hint", "content": "Track minIdx and maxIdx as you loop. Start with both at 0. Update minIdx when you find a smaller value, maxIdx when you find a larger value. Then swap at the end." }, { "title": "🔧 Pattern", "content": "<pre>1. Initialize minIdx = 0, maxIdx = 0\n2. Loop through slice:\n   - nums[i] < nums[minIdx]? → minIdx = i\n   - nums[i] > nums[maxIdx]? → maxIdx = i\n3. Swap: nums[minIdx], nums[maxIdx] = nums[maxIdx], nums[minIdx]</pre>" }], "solution": "func swapMinMax(nums []int) {\n    minIdx, maxIdx := 0, 0\n    for i := range nums {\n        if nums[i] < nums[minIdx] { minIdx = i }\n        if nums[i] > nums[maxIdx] { maxIdx = i }\n    }\n    nums[minIdx], nums[maxIdx] = nums[maxIdx], nums[minIdx]\n}" },
        { "id": "v8", "title": "Rotate Left by One", "description": "Write <code>func rotateLeft(nums []int)</code> that shifts all elements one position left, wrapping the first element to the end. E.g., [1,2,3,4] → [2,3,4,1].", "functionSignature": "func rotateLeft(nums []int)", "testCases": [{ "input": "[]int{1,2,3,4}", "output": "[2,3,4,1]" }, { "input": "[]int{5,10,15}", "output": "[10,15,5]" }], "hints": [{ "title": "🤔 Think about it", "content": "You need to save the first element, shift everything left, then put the saved element at the end." }, { "title": "💡 Hint", "content": "Save first := nums[0]. Loop from i=0 to len-2, copying nums[i+1] to nums[i]. Finally set nums[len-1] = first." }, { "title": "🔧 Pattern", "content": "<pre>1. Save first element: first := nums[0]\n2. Shift elements left: for i := 0; i < len-1; i++ → nums[i] = nums[i+1]\n3. Place first at end: nums[len-1] = first</pre>" }], "solution": "func rotateLeft(nums []int) {\n    if len(nums) == 0 { return }\n    first := nums[0]\n    for i := 0; i < len(nums)-1; i++ {\n        nums[i] = nums[i+1]\n    }\n    nums[len(nums)-1] = first\n}" },
        { "id": "v9", "title": "Swap Halves", "description": "Write <code>func swapHalves(nums []int)</code> that swaps the first half with the second half. For [1,2,3,4] → [3,4,1,2]. Assume even length.", "functionSignature": "func swapHalves(nums []int)", "testCases": [{ "input": "[]int{1,2,3,4}", "output": "[3,4,1,2]" }, { "input": "[]int{10,20,30,40,50,60}", "output": "[40,50,60,10,20,30]" }], "hints": [{ "title": "🤔 Think about it", "content": "You need to swap element 0 with element n/2, element 1 with element n/2+1, etc. How many swaps is that?" }, { "title": "💡 Hint", "content": "Loop from i=0 to len/2. Each iteration swaps nums[i] with nums[i + len/2]." }, { "title": "🔧 Pattern", "content": "<pre>1. Calculate mid := len(nums) / 2\n2. Loop i from 0 to mid:\n   - Swap nums[i] with nums[i + mid]\n</pre>" }], "solution": "func swapHalves(nums []int) {\n    mid := len(nums) / 2\n    for i := 0; i < mid; i++ {\n        nums[i], nums[i+mid] = nums[i+mid], nums[i]\n    }\n}" }
      ]
    },
    {
